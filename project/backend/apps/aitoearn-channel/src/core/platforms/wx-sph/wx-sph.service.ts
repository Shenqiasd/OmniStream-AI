...
@Injectable()
export class WxSphService {
  private readonly logger = new Logger(WxSphService.name)
  ...
  isAutomationConfigured(): boolean {
    return Boolean(config.wxSph?.mcpUrl?.trim())
  }

  private getMcpUrl(): string {
    return config.wxSph?.mcpUrl?.trim() || ''
  }

  private getMcpTimeoutMs(): number {
    return config.wxSph?.timeoutMs || 30000
  }
  ...
  private getMcpHeaders(sessionId?: string): Record<string, string> {
    ...
    if (config.wxSph?.mcpAuthToken) {
      headers.Authorization = `Bearer ${config.wxSph.mcpAuthToken}`
    }
    if (sessionId) {
  ...
  private async initializeMcpSession(): Promise<string | undefined> {
    ...
    if (initResponse.status >= 400) {
      throw new Error(`WxSph MCP initialize failed with status ${initResponse.status}`)
    }
  ...
  private async callMcpTool<T = unknown>(name: string, args: Record<string, unknown>): Promise<T> {
    if (!this.isAutomationConfigured()) {
      throw new Error('WxSph MCP is not configured')
    }
    ...
    if (response.status >= 400) {
      throw new Error(`WxSph MCP tool ${name} failed with status ${response.status}`)
    }
    if (response.data?.error) {
      throw new Error(response.data.error.message || `WxSph MCP tool ${name} failed`)
    }
  ...
  private normalizePublishResult(payload: unknown): PublishingTaskResult {
    ...
    if (!postId) {
      throw PublishingException.nonRetryable('WeChat Channels publish result did not include a post identifier')
    }
  ...
  private async resolveAccountSummary(accountId?: string) {
    ...
    try {
      ...
      this.logger.warn(`load wxSph account ${accountId} failed: ${error}`)
      return {
        id: accountId,
        found: false,
      }
  ...
  private async ensureSingleBoundAccount(accountId: string, userId: string) {
    ...
    if (!selectedAccount || selectedAccount.type !== AccountType.WxSph) {
      throw PublishingException.nonRetryable('Selected WeChat Channels account is not available')
    }

    const userAccounts = await this.serverClient.account.getUserAccounts(userId)
    const wxSphAccounts = userAccounts.filter(account => account.type === AccountType.WxSph)
    if (wxSphAccounts.length > 1) {
      throw PublishingException.nonRetryable(
        'WeChat Channels MCP bridge is single-session. Keep only one WeChat Channels account bound before publishing.',
      )
    }
  ...
  private async prepareLocalVideoPath(videoUrl: string) {
    ...
    const tempDir = resolve(process.cwd(), 'tmp', 'wx-sph-mcp')
  ...
  async checkSession(accountId?: string) {
    ...
    if (!this.isAutomationConfigured()) {
      return {
        ...base,
        valid: false,
        message: 'Set WXSPH_MCP_URL to enable WeChat Channels automation bridge. Login still happens in the external bridge.',
      }
    }
  ...
  async publishVideo(publishTask: PublishTask): Promise<PublishingTaskResult> {
    if (!publishTask.videoUrl) {
      throw PublishingException.nonRetryable('WeChat Channels publishing requires a videoUrl')
    }
