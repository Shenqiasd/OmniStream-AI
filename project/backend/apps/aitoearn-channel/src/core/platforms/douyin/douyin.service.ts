...
export class DouyinService {
  ...
  isAutomationConfigured(): boolean {
    return Boolean(config.douyin?.mcpUrl?.trim())
  }

  private getMcpUrl(): string {
    return config.douyin?.mcpUrl?.trim() || ''
  }

  private getMcpTimeoutMs(): number {
    return config.douyin?.timeoutMs || 30000
  }
  ...
  private getMcpHeaders(sessionId?: string): Record<string, string> {
    ...
    if (config.douyin?.mcpAuthToken) {
      headers.Authorization = `Bearer ${config.douyin.mcpAuthToken}`
    }
    if (sessionId) {
  ...
  private async initializeMcpSession(): Promise<string | undefined> {
    ...
    if (initResponse.status >= 400) {
      throw new Error(`Douyin MCP initialize failed with status ${initResponse.status}`)
    }
  ...
  private async callMcpTool<T = unknown>(name: string, args: Record<string, unknown>): Promise<T> {
    if (!this.isAutomationConfigured()) {
      throw new Error('Douyin MCP is not configured')
    }
    ...
    if (response.status >= 400) {
      throw new Error(`Douyin MCP tool ${name} failed with status ${response.status}`)
    }
    if (response.data?.error) {
      throw new Error(response.data.error.message || `Douyin MCP tool ${name} failed`)
    }
  ...
  private normalizePublishResult(payload: unknown): PublishingTaskResult {
    ...
    if (!postId) {
      throw PublishingException.nonRetryable('Douyin publish result did not include a post identifier')
    }
  ...
  private async resolveAccountSummary(accountId?: string) {
    ...
    try {
      ...
      this.logger.warn(`load douyin account ${accountId} failed: ${error}`)
      return {
        id: accountId,
        found: false,
      }
  ...
  private async ensureSingleBoundAccount(accountId: string, userId: string) {
    ...
    if (!selectedAccount || selectedAccount.type !== AccountType.Douyin) {
      throw PublishingException.nonRetryable('Selected Douyin account is not available')
    }

    const userAccounts = await this.serverClient.account.getUserAccounts(userId)
    const douyinAccounts = userAccounts.filter(account => account.type === AccountType.Douyin)
    if (douyinAccounts.length > 1) {
      throw PublishingException.nonRetryable(
        'Douyin MCP bridge is single-session. Keep only one Douyin account bound before publishing.',
      )
    }
  ...
  private async prepareLocalVideoPath(videoUrl: string) {
    ...
    const tempDir = resolve(process.cwd(), 'tmp', 'douyin-mcp')
  ...
  async checkSession(accountId?: string) {
    ...
    if (!this.isAutomationConfigured()) {
      return {
        ...base,
        valid: false,
        message: 'Set DOUYIN_MCP_URL to enable Douyin automation bridge. Login still happens in the external bridge.',
      }
    }
  ...
  async publishVideo(publishTask: PublishTask): Promise<PublishingTaskResult> {
    if (!publishTask.videoUrl) {
      throw PublishingException.nonRetryable('Douyin video publishing requires a videoUrl')
    }
